...
-(void)displayGameOverText {
    _gameOverTextLabel = [CCLabelTTF labelWithString:[NSString stringWithFormat:@"Record: %ld M", (long)[_userDefault integerForKey:@"record"]] fontName:@"Verdana-Bold" fontSize:18.0f];
    _gameOverTextLabel.positionType = CCPositionTypeNormalized;
    _gameOverTextLabel.position = ccp(0.6f, 0.7f);
    _gameOverTextLabel.color = [CCColor colorWithCcColor3b:ccc3(130, 22, 22)];
...
-(void)displayTotalCoins {
    _totalCoinLabel = [CCLabelTTF labelWithString:[NSString stringWithFormat:@"Coins: %ld", (long)[_userDefault integerForKey:@"numOfCoins"] ] fontName:@"Verdana-Bold" fontSize:18.0f];
    _totalCoinLabel.positionType = CCPositionTypeNormalized;
    _totalCoinLabel.position = ccp(0.6f, 0.55f);
    _totalCoinLabel.color = [CCColor colorWithCcColor3b:ccc3(130, 22, 22)];
    [self addChild:_totalCoinLabel z:0];
}

-(void)displayTotalScore {
    _totalScoreLabel = [CCLabelTTF labelWithString:[NSString stringWithFormat:@"Score: %i", _score] fontName:@"Verdana-Bold" fontSize:30.0f];
    _totalScoreLabel.positionType = CCPositionTypeNormalized;
    _totalScoreLabel.position = ccp(0.6f, 0.35f);
    _totalScoreLabel.color = [CCColor colorWithCcColor3b:ccc3(242, 235, 39)];
...
-(void)addRestartButton {
    _restartButton = [CCButton buttonWithTitle:@"[ Restart ]" fontName:@"Verdana-Bold" fontSize:18.0f];
    _restartButton.positionType = CCPositionTypeNormalized;
    _restartButton.position = ccp(0.3, 0.2f);
...
-(void)addMenuButton {
    _menuButton = [CCButton buttonWithTitle:@"[ Menu ]" fontName:@"Verdana-Bold" fontSize:18.0f];
    _menuButton.positionType = CCPositionTypeNormalized;
    _menuButton.position = ccp(0.7, 0.2f);
