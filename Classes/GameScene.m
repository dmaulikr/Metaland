...
-(void)generateRandomMissile {
    ...
    if(typeFlag<0.0f) {
        missile = [Enemies missileInit:MISSILE_GOLDEN];
        //[audio playEffect:@"warning.wav"];

        actionMove = [CCActionMoveBy actionWithDuration:2 position:ccp(-_background._background1.contentSize.width-2*missile.contentSize.width,0)];
...
-(void)calculateDistacne {
    ...
    [(CCLabelTTF*)[self getChildByName:@"labelDistance" recursively:NO] setString:[NSString stringWithFormat:@"Distance: %d M", _distance]];
}
